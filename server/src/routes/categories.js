...
const categoryTree = [];
const categoryMap = {}; categories.forEach(cat => {
            categoryMap[cat.id] = {
                ...cat,
                children: []
            };
...
router.get('/stats', async (req, res, next) => {
    try {
        const stats = await db.all(`
            ...
            LEFT JOIN transactions t ON ts.transaction_id = t.id AND t.status = 'LOCK
ED'
            LEFT JOIN categories p ON c.parent_id = p.id
            WHERE c.is_system_category = 0
            GROUP BY c.id, c.name, c.parent_id, p.nameORDER BY total_amount DESC
`);
res.json({ stats });
} catch (error) {
next(error);
